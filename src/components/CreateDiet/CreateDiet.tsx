...
import {Meal, Alimento, TipoAlimento, PersonData, AlimentoEntry, Equivalente} from "../../types/dietTypes";
import { createNewMeal, createNewAlimento } from "../../utils/dietUtils";
import { fetchTiposAlimento, fetchAlimentosEquivalentes } from "../../api/dietApi";
...
const CreateDiet: React.FC = () => {
    ...
    const handleAlimentoChange = useCallback((mealIndex: number, alimentoIndex: number, field: keyof AlimentoEntry, value: string | number | Alimento[] | Equivalente[]| number) => {
    ...
    const buscarAlimentosEquivalentes = useCallback(async (mealIndex: number, alimentoIndex: number) => {
        ...
        if (alimento.idAlimento && alimento.quantidade && alimento.tipoQuantidade) {
            ...
            const alimentosEquivalentes = equivalentes;

            // Passar apenas o array "equivalentes" (do tipo Equivalente[])
            handleAlimentoChange(mealIndex, alimentoIndex, 'equivalentes', alimentosEquivalentes);
        } else {
